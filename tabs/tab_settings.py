...
def tab_settings(st) -> None:
    ...
    # Bireysel Revize (Zamanlı) ayarları
    st.markdown("---")
    st.subheader("Bireysel Revize (Zamanlı)")
    try:
        conn_tmp = sqlite3.connect("ucus_egitim.db", check_same_thread=False)
        donemler = _list_donemler(conn_tmp)
    except Exception:
        donemler = []
        conn_tmp = None
    auto_cfg = config.get("auto_bireysel", {}) if isinstance(config.get("auto_bireysel", {}), dict) else {}
    with st.form("auto_bireysel_form"):
        enabled = st.toggle("Planlanan saatte bireysel revizeyi calistir", value=bool(auto_cfg.get("enabled", False)))
        sel_donem = st.selectbox(
            "Dönem",
            options=((([auto_cfg.get("donem")] if auto_cfg.get("donem") else []) + [d for d in donemler if d != auto_cfg.get("donem")]) if donemler else [auto_cfg.get("donem", "")]),
            index=0,
        )
        try:
            ogrenciler = _list_ogrenciler(conn_tmp, sel_donem) if conn_tmp and sel_donem else []
        except Exception:
            ogrenciler = []
        sel_ogr = st.selectbox(
            "Öğrenci",
            options=((([auto_cfg.get("ogrenci")] if auto_cfg.get("ogrenci") else []) + [o for o in ogrenciler if o != auto_cfg.get("ogrenci")]) if ogrenciler else [auto_cfg.get("ogrenci", "")]),
            index=0,
        )
        saved_auto = st.form_submit_button("Bireysel ayarları kaydet")
    try:
        if conn_tmp:
            conn_tmp.close()
    except Exception:
        pass
    if saved_auto:
        config["auto_bireysel"] = {"enabled": bool(enabled), "donem": str(sel_donem or ""), "ogrenci": str(sel_ogr or "")}
        _save_config(config)
        st.success("Bireysel revize ayarları güncellendi.")

    # Kalıcı mesaj
    st.markdown("---")
    try:
        from tabs.revize_panel_bireysel import panel as bireysel_panel
        _conn_inline = sqlite3.connect("ucus_egitim.db", check_same_thread=False)
        with st.expander("Bireysel Revize Paneli (Inline)", expanded=False):
            bireysel_panel(_conn_inline)
        try:
            _conn_inline.close()
        except Exception:
            pass
    except Exception as _exc:
        st.caption(f"Bireysel panel yuklenemedi: {_exc}")
    ...
    if should_display:
        ...
        try:
            ...
            # Zamanli bireysel revize calistir
            auto_cfg = config.get("auto_bireysel", {}) if isinstance(config.get("auto_bireysel", {}), dict) else {}
            if auto_cfg.get("enabled") and auto_cfg.get("ogrenci"):
                st.markdown("---")
                st.subheader("Bireysel Revize (Zamanli Calistirma)")
                st.caption(f"Secili ogrenci: {auto_cfg.get('ogrenci')} | Donem: {auto_cfg.get('donem')}")
                _auto_bireysel_execute(st, donem=auto_cfg.get("donem", ""), ogrenci=auto_cfg.get("ogrenci", ""))
        except Exception as exc:
